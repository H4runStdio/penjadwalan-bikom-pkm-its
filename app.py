...
def _sheet_tim_belum_terplot(wb: Workbook, df_tim: pd.DataFrame, terplot_nrp_bidang: set):
    ...
    for _, r in df_tim.iterrows():
        ...
        if key not in terplot_nrp_bidang:
            ...
            belum_terplot_rows.append(str(r["bidang pkm"]))
            row_idx += 1
            nomor   += 1
...
def main():
    st.set_page_config(
        page_title="Penjadwalan Bimbingan Komunal PKM ITS",
        page_icon=":calendar:",
        layout="wide",
    )
    ...
    st.markdown(
        ...
        "Sistem Penjadwalan Bimbingan Komunal PKM ITS</h1>",
    ...
    if st.button("Proses Penjadwalan", type="primary", use_container_width=True):
        ...
        with st.expander("Rekap per Hari & Bidang", expanded=False):
            ...
            st.markdown("**Rekap Tim Belum Terplot per Bidang PKM:**")
            terplot_set_ui = {(h["NRP"], h["Bidang"]) for h in hasil}
            belum_list = [
                str(r["bidang pkm"])
                for _, r in df_tim.iterrows()
                if (str(r["nrp"]), str(r["bidang pkm"])) not in terplot_set_ui
            ]
            from collections import Counter
            rekap_belum = Counter(belum_list)
            df_rekap_belum = pd.DataFrame(
                sorted(rekap_belum.items(), key=lambda x: x[0]),
                columns=["Bidang PKM", "Jumlah Tim Belum Terplot"],
            )
            st.dataframe(df_rekap_belum, hide_index=True, use_container_width=True)
